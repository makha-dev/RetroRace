...
private:
	...
	float fDistance = 0.0f, fCurvature = 0.0f;	
	float fTrackCurvature = 0.0f, fTrackDistance = 0.0f;

	float fCarPos = 0.0f,  fSpeed = 0.0f;
	...
	virtual bool OnUserUpdate(float fElapsedTime){
		...
		for (int y = 0; y < ScreenHeight() / 2; y++)
			for (int x = 0; x < ScreenWidth(); x++){
				...
				fRoadWidth *= 0.5f;	// Halve it as track is symmetrical around center of track
